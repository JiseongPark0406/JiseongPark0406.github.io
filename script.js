...
async function loadCSVData() {
  ...
  for (const path of possiblePaths) {
    try {
      ...
      // 인코딩 처리 - CP949/EUC-KR 우선, 실패시 UTF-8
      let decodedText;
      try {
        console.log("CP949 디코딩 시도...");
        const decoder = new TextDecoder("euc-kr");
        decodedText = decoder.decode(arrayBuffer);

        // 디코딩 결과 검증 (한글이 깨졌는지 확인)
        if (
          decodedText.includes("�") ||
          (!decodedText.includes("교과목명") && !decodedText.includes("전공"))
        ) {
          throw new Error("EUC-KR 디코딩 결과 의심스러움");
        }
        console.log("✅ EUC-KR 디코딩 성공");
      } catch (error) {
        try {
          console.log("EUC-KR 실패, UTF-8 시도...");
          const decoder = new TextDecoder("utf-8");
          decodedText = decoder.decode(arrayBuffer);
          console.log("✅ UTF-8 디코딩 성공");
        } catch (error2) {
          console.log("모든 인코딩 실패, 기본 디코더 사용");
  ...
  document.getElementById("courseList").innerHTML = `
    <div class="loading-message" style="color: red;">
      ❌ course_schedule_with_section_id.csv 파일을 찾을 수 없습니다!<br>
      <small>파일이 같은 폴더에 있는지 확인해주세요.</small><br><br>
      <button onclick="loadEmbeddedData()" style="padding: 10px 20px; background: #ffa502; color: white; border: none; border-radius: 5px; cursor: pointer;">
        🧪 테스트 데이터로 시작하기
      </button>
    </div>
  `;
...
function setupDataAndUI(source) {
  ...
  document.getElementById("courseList").innerHTML = `
    <div class="loading-message" style="color: ${color};">
      ...
      ${
        isEmbedded
          ? "<br><br><strong>참고:</strong> CSV 파일을 찾을 수 없어 테스트 데이터를 사용 중입니다."
          : ""
      }
    </div>
  `;
